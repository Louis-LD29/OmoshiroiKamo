...
public class RecipeConfig {
    ...
    public static RecipeConfig loadRecipeConfig(String coreFileName, String customFileName,
        ...
        try {
            ...
            if (userConfigStr == null || userConfigStr.trim()
                .length() == 0) {
    ...
    public static String readRecipes(File copyTo, String fileName, boolean replaceIfExists) throws IOException {
        ...
        InputStream in = RecipeConfig.class.getResourceAsStream("/assets/test/config/" + fileName);
        if (in == null) {
            Log.error("Could load default AlloySmelter recipes.");
            throw new IOException("Could not resource /assets/test/config/" + fileName + " form classpath. ");
        }
    ...
    public void merge(RecipeConfig userConfig) {
        ...
        for (RecipeGroup group : userConfig.getRecipeGroups()
            .values()) {
            if (!group.enabled) {
    ...
    public RecipeConfig.RecipeGroup createRecipeGroup(String name) {
        return new RecipeConfig.RecipeGroup(name);
    }
